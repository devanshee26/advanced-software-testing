...
valid_mutants=0
total_survived=0
rm -f ./Mutation_Testing_CP_EachBlocks_Survived.txt
while [ $i -ne $provided ]
do

    rm -rf *.gc* 
    rm -rf coverage_*.*
    g++ -std=c++14 -o ./SetCover_Mutation_TestSuite/SetCover_Mutation_Exec_$i ./Mutation_Testing/mutations_CodeOnly/SetCover_CodeOnly.mutant.$i.c ./SetCover_Mutation_TestSuite/SetCover_Mutation_EachBlockPassedTests.cpp $(pkg-config --cflags --libs gtest) -pthread
    ...
    if [ -f "./SetCover_Mutation_TestSuite/SetCover_Mutation_Exec_$i" ]; then
        ...
        if ps -p $pid > /dev/null; then
            ...
            if [ $exit_status -eq 0 ]; then
                echo "Mutation $i is survived" >> ./Mutation_Testing_CP_EachBlocks_Survived.txt
