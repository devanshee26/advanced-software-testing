...
valid_mutants=0
total_survived=0
rm -rf ../executables_base/* 
while [ $i -ne $provided ]
do

    rm -rf *.gc* 
    rm -rf coverage_*.*
    g++ -std=c++14 -o ../executables_base/SetCover_Mutation_Exec_$i ../mutations_CodeOnly/SetCover_CodeOnly.mutant.$i.c ../../SetCover_Mutation_TestSuite/SetCover_Mutation_BaseBlockPassedTests.cpp $(pkg-config --cflags --libs gtest) -pthread
       
    "../executables_base/SetCover_Mutation_Exec_$i" &
    pid=$!
    if [ -f "../executables_base/SetCover_Mutation_Exec_$i" ]; then
        ...
        if ps -p $pid > /dev/null; then
            ...
            "../executables_base/SetCover_Mutation_Exec_$i" > /dev/null 2>&1
            exit_status=$?
            ...
            "../executables_base/SetCover_Mutation_Exec_$i" > "../outputs_base/Output_Mutation_$i.txt"
        fi
